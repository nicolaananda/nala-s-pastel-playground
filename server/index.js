...
const participantInput=(body={})=>({participantName:String(body.participantName||'').trim(),birthDate:String(body.birthDate||''),schoolName:String(body.schoolName||'').trim(),parentName:String(body.parentName||'').trim(),whatsapp:normalizeWhatsapp(body.whatsapp),email:String(body.email||'').trim().toLowerCase()});
const validParticipant=(p)=>p.participantName.length<=255&&p.participantName.length>0&&/^\d{4}-\d{2}-\d{2}$/.test(p.birthDate)&&!Number.isNaN(Date.parse(p.birthDate))&&p.parentName.length>0&&p.parentName.length<=255&&p.schoolName.length<=255&&p.whatsapp.length>=10&&p.whatsapp.length<=20&&validEmail(p.email)&&p.email.length<=255;
...
app.get('/api/admin/competition-registrations',requireAdmin,async(req,res)=>res.json({registrations:await db.getCompetitionRegistrations(req.query.competitionId,req.query.archived==='true')}));
app.get('/api/admin/competitions/:competitionId/participants',requireAdmin,async(req,res)=>res.json({registrations:await db.getCompetitionRegistrations(req.params.competitionId,req.query.archived==='true')}));
app.post('/api/admin/competitions/:competitionId/participants',requireAdmin,async(req,res)=>{const competition=(await db.getAdminContent('competition')).find(c=>String(c.id)===req.params.competitionId);const p=participantInput(req.body);if(!competition)return res.status(404).json({message:'Kompetisi tidak ditemukan'});if(!validParticipant(p))return res.status(400).json({message:'Data peserta tidak valid'});const suffix=crypto.randomBytes(5).toString('hex').toUpperCase();const orderId=`ADMIN-${Date.now()}-${suffix}`;let registration=await db.createCompetitionRegistration({...p,competitionId:competition.id,competitionTitle:competition.title,orderId,registrationCode:`NALA-${suffix}`,accessToken:crypto.randomBytes(24).toString('hex'),bookProofUrl:null,amount:0,expiresAt:new Date()});registration=await db.updateCompetitionPayment(orderId,'paid','ADMIN-MANUAL');await db.logAdminAction({adminEmail:req.admin.email,action:'create_manual_participant',entityType:'competition_registration',entityId:String(registration.id),details:{competitionId:competition.id}});res.status(201).json({registration});});
app.patch('/api/admin/competitions/:competitionId/participants/:id',requireAdmin,async(req,res)=>{const p=participantInput(req.body);if(!validParticipant(p))return res.status(400).json({message:'Data peserta tidak valid'});const registration=await db.updateCompetitionRegistration(req.params.competitionId,req.params.id,p);if(!registration)return res.status(404).json({message:'Peserta aktif tidak ditemukan'});await db.logAdminAction({adminEmail:req.admin.email,action:'update_participant',entityType:'competition_registration',entityId:String(registration.id),details:{competitionId:req.params.competitionId,fields:Object.keys(p)}});res.json({registration});});
app.post('/api/admin/competitions/:competitionId/participants/:id/archive',requireAdmin,async(req,res)=>{const registration=await db.archiveCompetitionRegistration(req.params.competitionId,req.params.id);if(!registration)return res.status(404).json({message:'Peserta tidak ditemukan'});await db.logAdminAction({adminEmail:req.admin.email,action:'archive_participant',entityType:'competition_registration',entityId:String(registration.id),details:{competitionId:req.params.competitionId}});res.json({registration});});
app.post('/api/admin/competitions/:competitionId/participants/:id/check-in',requireAdmin,async(req,res)=>{const registration=await db.checkInCompetitionRegistration(req.params.competitionId,req.params.id);if(!registration)return res.status(409).json({message:'Peserta aktif belum lunas atau tidak ditemukan'});await db.logAdminAction({adminEmail:req.admin.email,action:'check_in_participant',entityType:'competition_registration',entityId:String(registration.id),details:{competitionId:req.params.competitionId}});res.json({registration});});
app.get('/api/admin/competitions/:competitionId/participants.csv',requireAdmin,async(req,res)=>{const rows=await db.getCompetitionRegistrations(req.params.competitionId,req.query.archived==='true');const safe=v=>{const s=String(v??'');return `"${(/^[=+\-@]/.test(s)?"'":'')+s.replace(/"/g,'""')}"`};const columns=['registrationCode','competitionTitle','participantName','birthDate','schoolName','parentName','whatsapp','email','bookProofUrl','amount','paymentStatus','transactionId','checkedInAt','archivedAt','createdAt'];await db.logAdminAction({adminEmail:req.admin.email,action:'export_participants',entityType:'competition',entityId:req.params.competitionId,details:{archived:req.query.archived==='true',count:rows.length}});res.type('text/csv').attachment(`peserta-lomba-${req.params.competitionId}.csv`).send('\ufeff'+[columns.join(','),...rows.map(r=>columns.map(c=>safe(r[c])).join(','))].join('\n'));});
